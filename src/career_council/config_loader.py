...
Defaults ship inside the package (`career_council/defaults/`). Users override any
subset by dropping files into `~/.career-council/`. Model ids are validated
...
from career_council.env import credential_env_names, hydrate_persistent_env
from career_council.input import PersonaSpec, VALID_MODES

USER_CONFIG_DIR = Path(os.path.expanduser("~")) / ".career-council"
...
def _read_default(filename: str) -> dict:
    if _resource_files is not None:
        text = _resource_files("career_council.defaults").joinpath(filename).read_text(encoding="utf-8")
