"""`career-council` command-line interface.
...
from career_council import __version__
from career_council.backends import BackendError, BackendRegistry
from career_council.config_loader import load_config, resolve_models, select_personas, validate_model_params
from career_council.input import CouncilInput, VALID_MODES
from career_council.env import hydrate_persistent_env
from career_council.metering import USAGE_LOG_PATH, summarize
from career_council.runner import CouncilPlan, plan_council, run_council
from career_council.sdk_client import SdkUnavailableError, discover_models
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="career-council", description="Multi-model Career Council runner.")
    parser.add_argument("--version", action="version", version=f"%(prog)s {__version__}")
    sub = parser.add_subparsers(dest="command")

    run_p = sub.add_parser("run", help="Convene the council on a brief.")
    run_p.add_argument("--mode", required=True, choices=list(VALID_MODES), help="RESUME | STRATEGY | INTERVIEW | OFFER.")
    run_p.add_argument("--stakes", default="standard", help="Tier: quick | standard | thorough (default: standard).")
    run_p.add_argument("--cwd", default=".", help="Folder holding your materials (resume, JD, notes) that advisors read (default: current dir).")
