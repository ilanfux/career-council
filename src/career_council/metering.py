...
Every agent run (advisor, peer reviewer, career director) appends one JSONL row
to `~/.career-council/usage.jsonl`. This is the data behind the budget trial: it
never blocks or breaks a council run, and it requires no server.
...
USER_DATA_DIR = Path(os.path.expanduser("~")) / ".career-council"
...
class MeteringSink:
    ...
    def record(
        ...
        if self.forward_db:
            _forward_to_fit_db(service_key=f"career-council.{self.mode}.{stage}.{persona_key}", model_name=model)
